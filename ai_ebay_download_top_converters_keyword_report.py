"""
eBay Top Converters Keyword Report Download (Playwright)
Goes to the Top Converters campaign, clicks Generate Report,
checks the Keyword report checkbox, clicks Generate, then
...
def download_keyword_report(campaign_id=CAMPAIGN_ID):
    """
    1. Go to campaign dashboard
    2. Click Generate Report
    3. Check the Keyword report checkbox
    4. Click Generate
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            url = f"https://www.ebay.com/sh/ads/dashboard/campaign/{campaign_id}"
            ...
            ss_popup = os.path.join(DOWNLOAD_DIR, f"tc_popup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png")
            page.screenshot(path=ss_popup, full_page=True)
            log.info(f"Popup screenshot: {ss_popup}")

            # Dump popup HTML for debugging
            html_dump = os.path.join(DOWNLOAD_DIR, f"tc_popup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.html")
            ...
            log.info(f"Popup HTML: {html_dump}")

            # 3. Check the "Keyword report" checkbox
            log.info("Looking for Keyword report checkbox...")
            keyword_checked = False

            # Try multiple approaches to find and check the keyword checkbox
            keyword_selectors = [
                "text='Keyword report'",
                "text='Keyword Report'",
                "text='keyword report'",
                "label:has-text('Keyword')",
                "span:has-text('Keyword report')",
                "div:has-text('Keyword report') >> input[type='checkbox']",
            ]
            ...
            for selector in keyword_selectors:
                try:
                    ...
                    if loc.is_visible(timeout=2000):
                        loc.click()
                        log.info(f"Clicked Keyword report option: {selector}")
            ...
            if not keyword_checked:
                # Try finding any checkbox near "Keyword" text
                checkboxes = page.locator("input[type='checkbox']").all()
                for cb in checkboxes:
                    try:
                        # Check the parent/sibling text
                        parent = cb.locator("..")
                        text = parent.inner_text().lower()
                        if "keyword" in text:
                            cb.check()
                            log.info(f"Checked keyword checkbox via parent text: {text.strip()[:50]}")
                            keyword_checked = True
                            break
                    except Exception:
                        continue
            ...
            if not keyword_checked:
                log.warning("Could not find Keyword report checkbox -- check screenshot")
            ...
            ss_after = os.path.join(DOWNLOAD_DIR, f"tc_after_generate_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png")
                ...
                for selector in [
                    ...
                    if loc.is_visible(timeout=1000):
                        ...
                        try:
                            ...
                            filename = f"Top Converters Test_Keyword_{today}.csv"
            ...
            if not download_clicked:
                ss_final = os.path.join(DOWNLOAD_DIR, f"tc_no_download_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png")
            ...
            ss_path = os.path.join(DOWNLOAD_DIR, f"tc_screenshot_error_{ts}.png")
            try:
